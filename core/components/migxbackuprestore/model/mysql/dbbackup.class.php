...
class DBBackup
{
     ...
     * @return void
     */
    public function purge($seconds = 1814400, $path_name = 'base_path')
    { // 21 days is the default
        ...
        while ($tmp_file = readdir($open_dir)) {
            if ($tmp_file != '.' && $tmp_file != '..') {
                ...
                # else files
                    ...
                    if ($path_name == 'temp_path' && is_file($data_folder . $tmp_file)) {
    ...
    protected function _generate()
    {
        ...
        foreach ($this->tables as $tbl) {
            ...
            if ($this->config['write_table_files']) {
                ...
                // copy file into file:
                file_put_contents($dir . $tbl['name'] . '.sql', $data, FILE_APPEND);
                $this->filePathData['tables'][$tbl['name']] = $dir . $tbl['name'] . '.sql';
            }
            // added 1.1.6:
            if ($this->config['write_file']) {
                file_put_contents($dir . $filename, $table_sql, FILE_APPEND);
                // copy file into file:
        ...
        if ($this->config['write_file']) {
            file_put_contents($dir . $filename, $this->config['comment_prefix'] . ' THE END' . $this->config['new_line'] . $this->config['comment_suffix'] . $this->config['new_line'], FILE_APPEND);
            $this->filePathData['database'] = $dir . $filename;
        }
    }
    /**
    ...
    protected function _getColumns($tableName)
    {
        ...
        try {
            ...
            //Todo: Move hardcoded table-settings to MIGX-Configuration (create special Configurations for each table)
            $this->config['table_settings']['modx_system_settings']['key_field'] = 'key';
            $this->config['table_settings']['modx_system_settings']['client_prefix'] = 'client_';

            $all_table_settings = isset($this->config['table_settings']) ? $this->config['table_settings'] : null;
            $table_settings = !empty($all_table_settings) && isset($all_table_settings[$tableName]) ? $all_table_settings[$tableName] : null;
            $key_field = !empty($table_settings) && isset($table_settings['key_field']) ? $table_settings['key_field'] : 'id';
            $client_prefix = !empty($table_settings) && isset($table_settings['client_prefix']) ? $table_settings['client_prefix'] : false;
            ...
            if ($client_prefix && strstr($create_stmt, '`' . $key_field . '`')) {
                //we have a table with the key - field
                //backup and restore custom-records, where the value is prefixed with a special prefix                
                $restore_custom = true;
            } elseif ($this->config['custom_autoinc'] && strstr($create_stmt, '`' . $key_field . '`')) {
            ...
            if ($restore_custom) {
                ...
                if ($client_prefix){
                    $sql .= 'INSERT INTO `tmp_' . $tableName . '` SELECT * FROM `' . $tableName . '` where `' . $key_field . '` LIKE "' . $client_prefix . '%";' . $this->config['new_line'];    
                }else{
                    $sql .= 'INSERT INTO `tmp_' . $tableName . '` SELECT * FROM `' . $tableName . '` where `' . $key_field . '` >= ' . $this->config['custom_autoinc'] . ';' . $this->config['new_line'];
                    //only on setup?
    ...
    protected function _getData($tableName)
    {
        try {
            ...
            $data = array();
            foreach ($q as $pieces) {
                $data[$count] = 'INSERT IGNORE INTO `' . $tableName . '` VALUES ( '; //.' (\'' . implode('\',\'', $pieces) . '\');'.$this->config['new_line'];
                $str = '';
